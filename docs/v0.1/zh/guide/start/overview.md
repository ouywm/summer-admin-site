...
`summerrs-admin` 是一套**完全用 Rust 写**的生产级后台管理系统,构建在 [Summer 框架](https://github.com/ouywm/spring-rs)(Spring 风格的 Rust 应用骨架) 之上。它将身份鉴权、多租户、AI 网关、消息推送、对象存储、声明式审计等能力以**插件组合**的形式集成,开箱即用、按需启用。
...
- 想先跑起来 → [安装环境](./installation) → [Docker 一键启动](./docker)
