...
仓库根目录的 `docker-compose.yml` 包含 5 个服务:
...
| `ratchjob` | `qingpan/ratchjob:stable` | 8725 / 8825 / 8925 | 任务调度（XXL-Job 兼容） |
...
> 容器内 `app` 服务通过 service 名 `postgres` / `redis` / `rustfs` / `ratchjob` 互通,所以 `S3_ENDPOINT` 要写 `http://rustfs:9000` 而不是 `localhost`。
